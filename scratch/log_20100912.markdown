...
3. User, UserStat, and Event (Action?) models  (AR or DM?)
4. 'Virtual' model for badges


## Another look at Github's existing model

Github has three main user 'feeds', the __News Feed__, __Your Actions__, and __Pull Requests__.  The first two correspond to actions by others on your repo's (or repo's you watch), and actions you have taken.
